...
from random import random, uniform, gammavariate
...
def set_card() -> None:
    ...
    card_info = inquirer.prompt([
        ...
        inquirer.Text("number", message="신용카드 번호 (하이픈 제외(-), Enter: 완료, Ctrl-C: 취소)", default=card_info["number"]),
        inquirer.Text("password", message="카드 비밀번호 앞 2자리 (Enter: 완료, Ctrl-C: 취소)", default=card_info["password"]),
        inquirer.Text("birthday", message="생년월일 (YYMMDD) / 사업자등록번호 (Enter: 완료, Ctrl-C: 취소)", default=card_info["birthday"]),
        inquirer.Text("expire", message="카드 유효기간 (YYMM, Enter: 완료, Ctrl-C: 취소)", default=card_info["expire"])
    ])
    if card_info:
...
# 재시도 대기 함수 (혼합분포 + 5% 스파이크)
def _sleep():
    # 70% 확률로 감마분포, 30% 확률로 균등분포
    if random() < 0.7:
        # 감마(shape=4, scale=0.25) + 최소 0.5초
        delay = gammavariate(4, 0.25) + 0.5
    else:
        # 균등분포 2~5초 사이
        delay = uniform(2, 5)
...
def _handle_error(ex, msg=None):
    ...
    if isinstance(ex, SRTNetFunnelError):
        delay = round(uniform(1, 4), 2)
        print(f"SRTNetFunnelError Waiting {delay:.2f}s before retrying...")
        time.sleep(delay)
        return True
    return False
